...
const OrganizadorEvento = () => {
  ...
  const load = async () => {
    ...
    const { data: tks } = await supabase
      .from("tickets" as any)
      .select("id, code, status, holder_name, holder_email, holder_phone, issued_at, used_at")
      .eq("event_id", id!)
  ...
  const filtered = useMemo(() => {
    ...
    if (q) {
      list = list.filter((t) =>
        [t.holder_name, t.holder_email, t.holder_phone, t.code].some((v) => v?.toLowerCase().includes(q)),
      );
    }
    return list;
  ...
  const exportCSV = () => {
    const headers = ["Nome", "E-mail", "Telefone", "Código", "Resgatado em", "Status", "Utilizado em"];
          ...
          <CardHeader>
            ...
            <div className="flex flex-wrap gap-3 mt-4">
              <div className="relative flex-1 min-w-[220px]">
                ...
                <Input className="pl-9" placeholder="Buscar por nome, e-mail, telefone ou código…" value={search} onChange={(e) => setSearch(e.target.value)} />
              </div>
